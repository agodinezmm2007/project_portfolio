...
var LDAvis = function(to_select, data_or_file_name, color1, color2) {
    ...
    var K, // number of topics
        ...
        lambda = {
		 ...
		 // --------- for THE ADDED TOPIC LABEL NAMESONE ---------
		topicNames = [],
		// --------- nomore for THE ADDED TOPIC LABELS  ONE -----------
    ...
    function visualize(data) {
		...
		// ----------- fuse for THE ADDED TOPIC LABEL NAMES ONE ------------
		var cut_names=0;
        ...
        function topic_on(circle) {
            ...
            d3.select("#" + barFreqsID)
                ...
                // MODIFIED LINE BELOW
                .text("Top-" + R + " Most Relevant Terms for Topic " + topics + ": " + topicNames[topics-1] + " (" + Freq + "% of tokens)");
            // grab the bar-chart data for this topic only:
            var dat2 = lamData.filter(function(d) {
